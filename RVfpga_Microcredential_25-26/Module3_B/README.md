...
Test on the board the exercise shown in the video from time 0:0 to time 6:25. Do the following steps:
   ...
   * Run the program in RVfpga-ViDBo and on the Nexys A7 board (if you have it).
      * RVfpga-Nexys (FPGA board): Set the path for the bitstream in the ```platformio.ini``` file as follows: ```board_build.bitstream_file = /home/rvfpga/RVfpga_MasterUCLM/src/rvfpganexys.bit```
      * RVfpga-ViDBo (simulator): Set the path for the simulator in the ```platformio.ini``` file as follows: ```board_debug.verilator.binary = /home/rvfpga/RVfpga_MasterUCLM/verilatorSIM_ViDBo/OriginalBinaries/RVfpga-ViDBo_Ubuntu22```
