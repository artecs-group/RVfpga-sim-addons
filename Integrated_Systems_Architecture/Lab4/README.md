...
## Exercise 2
Follow the instructions explained at time 18:20 of the video provided in step 3 above ([PerformanceBenchmarkingVideo](https://youtu.be/GqaDEW3W4X0?si=yf1rObPveS-RB-We&t=1100)) in order to test, on the FPGA board and in RVfpga-ViDBo, the extended core with the simple example that does a floating point addition (note that in the video the example is tested in RVfpga-ViDBo). 
...
In this exercise you will test a program with floating point operations, using the extended SoC. Follow the next steps:
   ...
   3. Execute the program on the FPGA or in the RVfpga-ViDBo simulator: Compare performance of the dot product in FP emulation vs FP implementation.
...
- Replace the following file: ```~/.platformio/platforms/chipsalliance/builder/main.py``` for the one that you can download here: [main.py](https://drive.google.com/file/d/1dlx2YjdlljqLzgwz1vNzZ9FJVIOsBjbs/view?usp=sharing)
  
Then, do the following tasks:

1. Execute the provided program in RVfpga-ViDBo (this cannot be done on the FPGA board) using the extended SoC (which you obtained in the previous tasks). In file ```platformio.ini``` you can see the ```build_flags``` set as follows, which enable the Zfinx extension in the compiler:
